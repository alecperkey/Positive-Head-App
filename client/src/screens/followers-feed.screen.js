...
class FollowersFeedScreen extends Component {
  ...
  render() {
    ...
    if (loading || !user || !user.followersTweetFeed) {
      return (
        <FlatList
          data={[1, 2, 3]}
    ...
    if (user && !user.followersTweetFeed.edges.length) {
      return (
        <View style={styles.container}>
          <Header onPress={this.goToNewGroup} />
          <Text style={styles.warning}>{'You have no followers anyone yet.'}</Text>
        </View>
      );
    }

    const tweetData = [];
    if (user.followersTweetFeed.edges.length) {
      forEach(user.followersTweetFeed.edges, (edge) => {
